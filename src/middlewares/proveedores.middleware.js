...
exports.validateProveedor = async  (req, res, next) => {
    ...
    const exists = await Proveedores.ProveedoresExists(nombre, apellido, cedula);
    if (exists) {
        return res.status(400).json({ message: 'Ya existe un proveedor con el mismo nombre, apellido y cédula' });
    }
